...
5.  **Mixins** Prioritize `@WrapOperation` where possible to avoid compatibility issues with other mods.
6.  **Changelogs:** Update the changelog after changes. The changelog is intended for final users; keep descriptions clear and user-focused. Use the **Back-end** category for technical or internal changes that users likely won't understand. Compact multiple changes into one sentence when possible.
